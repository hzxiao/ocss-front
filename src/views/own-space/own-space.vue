...
<script>
    ...
    import {StudentApi, TeacherApi, UserApi} from '../../api'
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                ...
                this.uid = Cookies.get('user');
                this.userForm.id = this.uid;
                switch (this.role) {
                    case '1':

                        break;
                    case '2':
                        TeacherApi.getById(this.uid).then(({data}) => {
                            if (data.code === this.$code.SUCCESS) {
                                this.userForm = util.safe(data, 'data.teacher', {});
                            } else {
                                return this.$Message.error(data.msg)
                            }
                        });
                        break;
                    case '3':
                        StudentApi.getById(this.uid).then(({data}) => {
                            if (data.code === this.$code.SUCCESS) {
                                this.userForm = util.safe(data, 'data.student', {});
                            } else {
                                return this.$Message.error(data.msg)
                            }
                        });
            ...
            saveInfoAjax() {
                ...
                switch (this.role) {
                    case '2':
                        TeacherApi.update(this.userForm).then(({data}) => {
                            if (data.code === this.$code.SUCCESS) {
                                this.$Message.success('修改成功')
                    ...
                    case '3':
                        StudentApi.update(this.userForm).then(({data}) => {
                            if (data.code === this.$code.SUCCESS) {
                                this.$Message.success('修改成功')
                                this.save_loading = false;
                            } else {
